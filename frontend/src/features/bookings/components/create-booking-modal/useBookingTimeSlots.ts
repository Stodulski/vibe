...
/** Every start on the day's grid that this court is actually free for. */
function availableStartTimes(
  date: string,
  courtId: string,
  existingBookings: Booking[] | undefined,
...
export function useBookingTimeSlots({
  ...
  return { timeSlots: availableStartTimes(date, courtId, existingBookings, durationMinutes) };
}
