...
function cancelBookingInfoOf(selectedBooking: Booking | null) {
...
function useSelectedBooking(complexId: string | null) {
  ...
  const select = (booking: Booking | null) => {
...
function useManualRefundHandlers(
  ...
  const handleManualRefundOpen = (booking: Booking, amount: number) => {
...
export function useBookingModals(selectedDate: string, complexId: string | null = null) {
  ...
  const cancelBookingInfo = cancelBookingInfoOf(selectedBooking);
  ...
  const handleCreateFromSlot = (prefill: CreateBookingPrefill) => {
    setCreatePrefill(prefill);
    setCreateOpen(true);
  };

  const handleCancelBooking = closeDetailAndOpen(setSelectedBooking, setDetailOpen, setCancelOpen);
  const handleConfirmPaymentOpen = closeDetailAndOpen(setSelectedBooking, setDetailOpen, setPaymentOpen);
