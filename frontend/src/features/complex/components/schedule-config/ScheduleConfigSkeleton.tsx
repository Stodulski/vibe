  ...
  return (
    <div role="status" aria-label={t.common.loading}>
      {Array.from({ length: 7 }, (_, i) => (
        <div key={i} className="flex items-center gap-x-4 gap-y-2 border-b border-border-subtle py-3 last:border-0">
          <Skeleton className="h-4 w-20 shrink-0 rounded-lg sm:w-28" />
          <Skeleton className="h-8 flex-1 rounded-lg sm:flex-none sm:w-40" />
