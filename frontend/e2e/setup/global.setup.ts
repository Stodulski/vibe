...
// backend) so an unset-env local run behaves exactly as before.
// `make e2e` in backend overrides BACKEND_URL to its isolated API
  ...
  try {
    execFileSync(
      ...
      [
        ...
        '-d',
        DB_NAME,
        // Kept in sync with db/migrations in backend. This used to include
