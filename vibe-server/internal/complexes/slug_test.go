...
//
// It cannot read vibe-client from here — a Go test in one repository has no
...
func TestReservedSlugsMatchTheKnownClientRoutes(t *testing.T) {
	...
	if len(reservedSlugs) != len(want) {
		t.Errorf("reservedSlugs has %d entries and this test knows %d: if a route was added to vibe-client's router, add it to both; if one was removed, remove it from both",
