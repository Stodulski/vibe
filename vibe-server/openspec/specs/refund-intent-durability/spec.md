...
booking durably `status='cancelled'` with `collection_status` still
`'deposit_paid'`/`'fully_paid'`, `refund_status` still `'none'`, and no
`failed_refunds` row — nothing in
...
collection status still paid and `refund_status='none'`, no attempt row. A client cancelling outside the
...
When a cancellation of a currently-paid booking (`collection_status` is
...
`refund_status='pending'`: `refundable()`
...
- GIVEN a paid booking (`collection_status='deposit_paid'` or `'fully_paid'`)
...
(`status='cancelled'`, collection status still paid, `refund_status='none'`,
no attempt row), because
...
- GIVEN a client cancels a paid booking (`collection_status='deposit_paid'`)
