...
moves a booking's `payment_status` may do so for a payment whose collector
...
`payment_status='refund_pending'` permanently once the retry budget
...
or updates a `data.Payment` row, or moves the booking's `payment_status`
toward `refund_pending`. A collector mismatch or an unverifiable collector
...
- WHEN `processApprovedPayment` routes into the already-cancelled branch for
  that webhook
- THEN no `data.Payment` row is written for the booking, its
  `payment_status` is not moved toward `refund_pending`, and
