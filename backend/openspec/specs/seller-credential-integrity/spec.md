...
exists now. This applies at minimum to the preference-expiry call sites:
`internal/bookings/cancel.go:104` and `cmd/api/cron.go:171`
(`cronReleaseExpiredPayments`).

**Delta supersedes this requirement's loud-alert-and-continue trade for
`getSellerToken` at its three call sites** (`AutoRefundIfPaid`,
`RetryFailedRefunds`, `refundCancelledBookingPayment`) — see the added
requirement below, "A seller-token failure that cannot self-heal refuses
before MercadoPago is called". The preference-expiry call sites keep the
original loud-alert-and-continue trade because they have no claim to refuse
into.
