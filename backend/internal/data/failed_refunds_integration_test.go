...
func (f *testFixture) createFailedRefund(t *testing.T, startTime, endTime string) *FailedRefund {
	t.Helper()
...
func idsOf(refunds []*FailedRefund) map[uuid.UUID]bool {
...
func TestTheRefundSweepReadsNoMoreThanARunCanWork(t *testing.T) {
	...
	for i := range refundSweepBatch + 3 {
...
func TestResolvedRefundAttemptsAreDeletedAndNothingElse(t *testing.T) {
	...
	store := &FailedRefundModel{DB: NewDB(f.Pool)}
